...
var (
	...
	outfileString   string = "short_vol_analyzer_output_" + time.Now().UTC().Format(DEFAULT_TIME_FORMAT) + ".json"
)
...
func init() {
	...
	flag.StringVar(&outfileString, "outfile", outfileString,
		"enter an output file name to write. Defaults to short_vol_analyzer_output_yyyy-mm-dd.json.")
}
...
func tickerCalculations(stockTickerData ShortData) (tickerFinal ShortData) {
	//initialize total variables inside the struct to zero so that we can get totals from all the days.
	tickerFinal = stockTickerData
	tickerFinal.TotalBuyVol = 0
	tickerFinal.TotalExemptShortVol = 0
	tickerFinal.TotalFloat = 0
	tickerFinal.TotalSharesOutstanding = 0
	tickerFinal.TotalSharesShort = float64(0)
	tickerFinal.TotalShortVol = 0
	tickerFinal.TotalVol = 0

	for _, tickerDateData := range stockTickerData.ShortVolumeData {
		tickerFinal.TotalSharesShort = tickerFinal.TotalSharesShort + (tickerDateData.ShortVol + tickerDateData.ShortExemptVol - tickerDateData.BuyVol)
		if tickerFinal.TotalSharesShort < 0 {
			tickerFinal.TotalSharesShort = 0
		}
		tickerFinal.TotalBuyVol += tickerDateData.BuyVol
		tickerFinal.TotalExemptShortVol += tickerDateData.ShortExemptVol
		tickerFinal.TotalShortVol += tickerDateData.ShortVol
		tickerFinal.TotalVol += tickerDateData.TotalVolume
	}
	return tickerFinal
}
...
func worker(tickers map[string]ShortData, start, end time.Time, deltaDays int64) (err error) {
	...
	*
	...
	fmt.Printf("Calculating totals for tickers.\n")
	for tickerString, ticker := range tickers {
		tickers[tickerString] = tickerCalculations(ticker)
	}
	return nil
}
...
func main() {
	...
	ioutil.WriteFile(outfileString, marshalIndentTickers, 0644)
